...
export function parseResetTime(text) {
  // Pattern 3: Weekly cap — "resets Jul 8 at 12am (America/New_York)"
  // Groups: [1]=monthDay, [2]=hour, [3]=minute?, [4]=ampm, [5]=tz
  let match = text.match(RESET_PATTERNS[2]);
  if (match) {
    ...
    return { type: 'weekly', hour, minute, timezone: match[5] };
  }

  // Pattern 4: "5-hour limit reached ... resets 9pm (America/New_York)"
  // Groups: [1]=limitNum, [2]=hour, [3]=minute?, [4]=ampm, [5]=tz
  match = text.match(RESET_PATTERNS[3]);
  if (match) {
    let hour = parseInt(match[2]);
    const minute = parseInt(match[3] || '0');
    const ampm = match[4]?.toLowerCase() || '';
    if (ampm === 'pm' && hour !== 12) hour += 12;
    if (ampm === 'am' && hour === 12) hour = 0;
    return { type: 'session', hour, minute, timezone: match[5] };
  }

  // Patterns 1-2: Session cap — "resets 4:20pm (Asia/Calcutta)"
  // Groups: [1]=hour, [2]=minute?, [3]=ampm, [4]=tz
  for (let i = 0; i < 2; i++) {
    match = text.match(RESET_PATTERNS[i]);
    if (!match) continue;
    ...
    if (ampm === 'pm' && hour !== 12) hour += 12;
    if (ampm === 'am' && hour === 12) hour = 0;
    return { type: 'session', hour, minute, timezone: match[4] };
  }
